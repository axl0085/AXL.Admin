...
namespace AXL.Admin.WebApi.Middleware {
    ...
    public class GlobalExceptionMiddleware {
        ...
        private static readonly Logger Logger = LogManager.GetCurrentClassLogger();//声明NLog变量

        public GlobalExceptionMiddleware(RequestDelegate next, ISysOperLogService sysOperLog) {
            this.next = next;
            SysOperLogService = sysOperLog;
        }
        ...
        private async Task HandleExceptionAsync(HttpContext context, Exception ex) {
            ...
            string ip = context.GetClientUserIp();
            var ip_info = IpTool.Search(ip);

            SysOperLog sysOperLog = new() {
                Status = 1,
                OperIp = ip,
                OperUrl = context.GetRequestUrl(),
            ...
            context.GetRequestValue(sysOperLog);
            ...
            ei.Properties["user"] = context.GetName();
